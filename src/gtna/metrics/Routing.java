...
// this.lcrl = new double[g.nodes.length];
// int[] rl = new int[0];
// this.prog = new double[0];
// int[] progCounter = new int[0];
// ArrayList<Integer> degreeOfFailures = new ArrayList<Integer>();
// for (int i = 0; i < g.nodes.length; i++) {
// lcrl[i] = 0;
// int success = 0;
// for (int j = 0; j < times; j++) {
...
// for (int k = 0; k < this.prog.length; k++) {
// temp2[k] = this.prog[k];
// }
// this.prog = temp2;
// // progCounter
// int[] temp3 = new int[p.length];
// for (int k = 0; k < progCounter.length; k++) {
// temp3[k] = progCounter[k];
// }
// progCounter = temp3;
// }
...
// }
// if (success > 0) {
// lcrl[i] /= (double) success;
// } else {
// degreeOfFailures.add(g.nodes[i].out().length * 2);
// }
// }
// if (degreeOfFailures.size() > 0) {
// System.out.println("====> " + degreeOfFailures.size());
// Plot.fast(new double[][] { Util.toDouble(Util.count(Util
// .toArray(degreeOfFailures))) }, new String[] { "network ("
// + degreeOfFailures.size() + ")" },
// "Degree of Failing nodes", "Degree d", "P(d)", "SPI-"
// + ra.folder() + ".pdf");
// }
// for (int i = 0; i < this.prog.length; i++) {
// this.prog[i] /= (double) progCounter[i];
// }
// this.fr /= (double) (g.nodes.length * times);
// Arrays.sort(this.lcrl);
// this.rld = Util.distribution(rl, times * g.nodes.length);
// this.crld = Util.cumulative(this.rld);
// int shortCount = Config.getInt("RL_LCRL_SHORT_MAX_VALUES");
// this.lcrlShort = Util.avgArray(this.lcrl, shortCount);
// this.crl = Util.avg(this.lcrl);
// this.mrl = rl.length;
// // TODO auslagern in anderes interface.....
// if (ra instanceof TwoPhaseGreedyRegistration) {
// this.copies = ((TwoPhaseGreedyRegistration) ra).copiesStored();
// Arrays.sort(this.copies);
// this.copiesShort = Util.avgArray(this.copies, Config
// .getInt("RL_COPIES_SHORT_MAX_VALUES"));
// this.averageCopies = Util.avg(this.copies);
// } else {
// this.averageCopies = 0.0;
// this.copies = new double[] { 0.0 };
// this.copiesShort = new double[] { 0.0 };
// }
// if (ra instanceof MessageCounter) {
// this.msgs = ((MessageCounter) ra).messages();
// Arrays.sort(this.msgs);
// this.msgsShort = Util.avgArray(this.msgs, Config
// .getInt("RL_MSGS_SHORT_MAX_VALUES"));
// this.avgMsgs = Util.avg(this.msgs);
// } else {
// this.msgs = new double[] { 0 };
// this.msgsShort = new double[] { 0 };
// this.avgMsgs = 0.0;
// }
// this.timer.end();
