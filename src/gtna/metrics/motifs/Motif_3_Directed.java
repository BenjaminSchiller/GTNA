...
import gtna.metrics.roles.ClearGraph;
import gtna.metrics.roles.GeneralGraphMethods;
import gtna.metrics.roles.Modules;
import gtna.metrics.roles.RoleFileReader;
...
//					.neighborhoodOfNode(actualNode, DIRECTEDMOTIF, graph);
			...
			for (int permutationNo = 0; permutationNo < combinations.size(); permutationNo++) {
...
//		MotifFunctions.outprintMotifs(motifsDirected, motif);
	...
	private Integer getMotifDirected(ArrayList<Integer> nodeList, Graph g) {
		...
		for (int i = 0; i < nodeList.size(); i++) {
			...
			// node
			degreeOfNode = GeneralGraphMethods.directedDegreeOfNodeWithinMotif(nodeList.get(i),
					nodeList, g);
	...
	public boolean writeData(String folder) {
		...
		for (int i = 0; i < motif3Total.length; i++) {
			motif3Total[i] = (double) ((double) motif3Total[i] / ((double) numberOfTotalMotifs / (double) 100));
		}
