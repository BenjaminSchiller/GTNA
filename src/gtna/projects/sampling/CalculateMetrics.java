...
//			System.out.println("Recognized name: " + name); // TODO REMOVE

			ReadableFolder rf = new ReadableFolder(name, name, p, suffix, null);

//			System.out.println("RF: " + d + " - size:" + rf.getFiles().length); // TODO
																				// REMOVE

			DescriptionWrapper dwrf = new DescriptionWrapper(rf, name+"-"+scaledown.get(j)+"%");
			rfc.add(dwrf);
			}
			...
			DescriptionWrapper dwrf = new DescriptionWrapper(rf, name+"-"+"100%");
			rfc.add(dwrf);
		}

		Network[] rfa = rfc.toArray(new Network[0]);

		Config.overwrite("SKIP_EXISTING_DATA_FOLDERS", "true"); // TODO really? idk
		
		Series[] series = new Series[rfa.length];
		for (int i = 0; i < rfa.length; i++) {
			String path;
			if(i>=scaledown.size()){
				path = targetdir + sampling + "/" + 100 + "/data";
			} else {
				path = targetdir + sampling + "/" + scaledown.get(i) + "/data";
			}
//			System.out.println("Setting MAIN_DATA_FOLDER=" + path); // TODO remove
			Config.overwrite("MAIN_DATA_FOLDER", path);

			series[i] = Series.generate(rfa[i], metrics.toArray(new Metric[0]),	times);
...
//				sizes.add(Integer.parseInt(s.substring(5)));
	...
	private static Collection<? extends Metric> matchMetrics(String s) {
		...
		for(String m : s.split(",")){
			Metric i;
			switch(m){
				case "CC":i = new ClusteringCoefficient(); break;
				case "DD":i = new DegreeDistribution(); break;
				case "SP":i = new ShortestPaths(); break;
				case "AS":i = new Assortativity(); break;
				case "BC":i = new BetweennessCentrality(); break;
				case "PR":i = new PageRank(); break;
				default: i = null;
			}
