...
/**
 * An EMatrix implementation using long for internal storage.
 * 
 * @author Philipp Neubrand
 * 
 */
public class MyEMatrixLong implements IMyEMatrix {
	 ...
	 *         algorithm
	 */
	public MyEMatrixLong(Graph graph) {
	...
	public String toString() {
		...
		for (int i = 0; i < dimension; i++) {
			if (!deleted[i]) {
				...
				for (int j = 0; j < dimension; j++) {
					if (!deleted[j]) {
						if (Long.toString(getValue(i, j)).length() > 7)
							ret.append(Long.toString(getValue(i, j)).substring(0, 6));
						else
