...
public class RolesGeneration extends TransformationImpl implements
	...
	public Graph transform(Graph g) {
		...
		for (GraphProperty gp : properties) {
			...
			Role[] roleOfNode = new Role[g.getNodes().length];

			for (Node node : g.getNodes()) {
				double z = getRelativeWithinModuleDegree(node.getIndex(), g,
						communities);
				double p = getParticipationCoefficient(node, communities);
				roleOfNode[node.getIndex()] = this.getRole(z, p);
			}
			g.addProperty(g.getNextKey("ROLES"), new Roles(roleOfNode));
		}
		return g;
	}
