 ...
 * ---------------------------------------
 * 2011-06-06 : changed some names (BS)
 *
 */
 ...
 *
 */
public class SwappingAttack extends TransformationImpl implements Transformation{
	...
	public SwappingAttack(int iterations, int attackers) {
		super("SWAPPING_ATTACK", new String[]{"ITERATIONS", "ATTACKERS"}, new String[]{""+iterations,""+attackers});
	...
	public Graph transform(Graph g) {
		...
		while (found < attackers){
			...
			if (!attacks[place]){
				...
				neighbors[found] = (RingNode)cur.out()[rand.nextInt(cur.out().length)];
				while (neighbors[found].out().length == 1){
					neighbors[found] = (RingNode)cur.out()[rand.nextInt(cur.out().length)];
				}
				pos[found] = neighbors[found].getID().pos;
				cur.getID().pos = neighbors[found].getID().pos+ rand.nextGaussian()*0.0000001;;
				level[found] = 2;
				for (int i = 0; i < imaginePos[found].length; i++){
					imaginePos[found][i] = cur.getID().pos +0.5+rand.nextGaussian()*0.0001;
				...
				while (corrupted < 10){
					steps++;
					partner = randomWalk(neighbors[found], rand.nextInt(level[found]-1)+1);
					...
					if (shouldSwap(cur,partner)){
						...
						cur.setID(new RingID(neighbors[found].getID().pos+ rand.nextDouble()*0.0000001));;
	...
	public void continueSort(){
		...
		while (count < iterations*curNodes.length){
			...
			if (attacks[cur.index()]){
				...
				partner = randomWalk(neighbors[atIndex], rand.nextInt(level[atIndex]-1)+1);
				while (partner.equals(cur)){
					partner = randomWalk(neighbors[atIndex], rand.nextInt(level[atIndex]-1)+1);
				}
				totalSt[atIndex][0]++;
				...
				if (shouldSwapCor(cur,imaginePos[atIndex],partner)){
					cur.getID().pos = pos[atIndex] + rand.nextDouble()*0.0000001;
						//neighbors[atIndex].getID().pos + rand.nextGaussian()*0.0000001;
				...
				boolean b;
				if (rep.size() > Math.pow(10, level[atIndex]-1) && level[atIndex] < 6){
					rep.removeFirst();
					int succ = 0;
					for (int i = 0; i < m; i++){
						b = rep.removeFirst();
					...
					if ((double)succ/m < ratio){
						level[atIndex]++;
						//System.out.println("Increased level at " +totalSt[atIndex][0] +" "+ totalSt[atIndex][1]);
	...
	public boolean shouldSwapCor(RingNode attacker, double[] pos, RingNode partner){
