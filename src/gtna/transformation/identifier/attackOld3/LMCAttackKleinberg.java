...
public class LMCAttackKleinberg extends LMCAttackNew {

	boolean[] attacks;
	int numAtt;
	Random rand = new Random();

	public LMCAttackKleinberg(int maxIter, int mode, double pRand,
			double delta, int C, boolean deg1, String file, int number) {
		super("LMC_ATTACK_KLEINBERG", maxIter, mode, pRand, delta, C, deg1,
				file, number);
		numAtt = number;
	...
	public RingID asked(RingNode node, RingNode cur, double min, Random rand,
		...
		if (this.modus == 2 && id.dist(newID) < min && before > after) {
			loc = newID.pos + min;
			while (loc > 1) {
				loc--;
			}
